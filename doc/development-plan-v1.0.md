...
- [ ] M2-001 班级 CRUD（软删除）。
- [ ] M2-002 学科与教师绑定模型。
- [ ] M2-003 关系一致性约束（外键/业务校验）。

## 3.2 学生 360 档案
- [ ] M2-010 学生 CRUD（含 `student_no + class_id` 唯一性策略）。
- [ ] M2-011 标签系统（增删改查 + 有效性校验）。
- [ ] M2-012 成绩记录时间序列。
- [ ] M2-013 观察记录与沟通历史关联视图。

## 3.3 Excel 批量导入
- [ ] M2-020 导入模板定义与字段映射。
- [ ] M2-021 行级错误回传（错误行号 + 原因 + 修复建议）。
- [ ] M2-022 导入去重与覆盖策略（明确“新增/跳过/更新”）。
- [ ] M2-023 导入审计日志（操作者、文件、结果统计）。
...
- [ ] M3-001 SQL 精确过滤（学生/班级/标签/时间）。
- [ ] M3-002 SQLite FTS 召回（观察/沟通文本）。
- [ ] M3-003 文件遍历补充（学生 memory Markdown）。
- [ ] M3-004 规则重排（近期优先、学科优先、高置信优先）。
- [ ] M3-005 Top-K 证据注入器（防上下文污染）。

## 4.2 学生长期记忆 Markdown 体系
- [ ] M3-010 固定目录结构创建：`/workspace/students/{student_id}/memory/`。
- [ ] M3-011 月度文件模板与 frontmatter 校验。
- [ ] M3-012 固定章节解析器。
- [ ] M3-013 高敏信息拦截（身份证号/住址等）。

## 4.3 家长沟通文案
- [ ] M3-020 Prompt 注入拼装（成绩趋势+标签+观察+历史语气）。
- [ ] M3-021 输出结构约束（先肯定→问题→建议）。
- [ ] M3-022 卡片操作（编辑/采纳/重生）。
- [ ] M3-023 采纳结果回写与证据可追溯。
...
## 4.5 班会/活动文案
- [ ] M3-040 单主题多对象文案（家长/学生/校内通知）。
- [ ] M3-041 校本模板套用。

验收：3 个典型场景之一“家长沟通文案/期末评语”闭环可演示。
...
## 5.1 作业图片批量结构化
- [ ] M4-001 前端拖拽仅传路径/句柄元数据（禁止 Base64 大包 IPC）。
- [ ] M4-002 后端按路径白名单读取文件。
- [ ] M4-003 图像预处理（去噪/矫正/分割）。
- [ ] M4-004 OCR 提取与题块定位。
- [ ] M4-005 学号姓名归并与自动核对。
- [ ] M4-006 导出 Excel（姓名/学号/题号/得分/置信度/冲突标记）。

## 5.2 增强模式批卷（可选增强）
- [ ] M4-010 接入多模态判卷（试卷 + 标准答案）。
- [ ] M4-011 OCR 结果与多模态判定融合。
- [ ] M4-012 冲突标记与低置信人工复核。
- [ ] M4-013 降级策略（模型不可用自动回落 OCR+规则）。

## 5.3 错题重组与专属练习
- [ ] M4-020 拉取近一月错题。
- [ ] M4-021 题目模板化（知识点/难度/题型）。
- [ ] M4-022 参数扰动生成同类题。
- [ ] M4-023 生成 Word 练习卷 + 答案页。

验收：3 个典型场景之一“批量作业结构化”闭环可演示。
...
## 6.1 AI 配置中心
- [ ] M5-001 Provider/Model 配置。
- [ ] M5-002 参数预设（严谨/创意）。
- [ ] M5-003 Keychain 集成（密钥不落盘明文）。

## 6.2 安全与隐私
- [ ] M5-010 存储目录与生命周期（导出/归档/擦除）。
- [ ] M5-011 外发前脱敏开关。
- [ ] M5-012 高危操作二次确认闸门。
...
## 6.4 Skills 与 MCP
- [ ] M5-030 内置 Skills 运行时（office/ocr/image/math/export）。
- [ ] M5-031 Python Skill 独立环境隔离（`~/.pureworker/skill-envs/...`）。
- [ ] M5-032 uv 健康检查（PATH + 回退路径 + 绝对路径复验）。
- [ ] M5-033 一键安装/修复 uv（用户明确触发，禁止静默安装）。
- [ ] M5-034 MCP 注册、权限声明、健康检查。
